...
```bash
0 0 * * * bash zImageOptimizer.sh -p /first/directory -q -n
0 1 * * * bash zImageOptimizer.sh -p /second/directory -q -n
```

Using custom time marker path and filename:
```bash
0 0 * * * bash zImageOptimizer.sh -p /first/directory -q -n -m /path/to/first/marker/firstMarkerName
0 1 * * * bash zImageOptimizer.sh -p /second/directory -q -n -m /path/to/second/marker/secondMarkerName
```

Also you may collect all markers in own directory:
```bash
0 0 * * * bash zImageOptimizer.sh -p /first/directory -q -n -m /path/to/markers/directory/firstMarkerName
0 1 * * * bash zImageOptimizer.sh -p /second/directory -q -n -m /path/to/markers/directory/secondMarkerName
```
